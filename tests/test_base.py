...
from migrate_wrapper import (
    MigrateWrapper, MigrateConfig, MigrationResult,
    DatabaseInfo, Migration, MigrateError, MigrateDirtyError
)
...
class MigrateWrapperTestBase(unittest.TestCase, ABC):
    ...
    def create_test_migration(self, version: int, name: str, up_sql: str, down_sql: str):
...
class MigrateWrapperTestMixin:
    ...
    def test_create_failure(self, mock_run):
        ...
        mock_run.return_value = MagicMock(
            returncode=1,
            stdout="",
            stderr="Failed to create migration"
        )
    ...
    def test_up_with_steps(self, mock_run):
        ...
        result = self.wrapper.up(steps=2)
    ...
    def test_up_dirty_state(self, mock_run):
        ...
        mock_run.return_value = MagicMock(
            returncode=1,
            stdout="",
            stderr="error: Dirty database version"
        )
    ...
    def test_up_already_latest(self, mock_run):
        ...
        mock_run.return_value = MagicMock(
            returncode=1,
            stdout="",
            stderr="no change: already at the latest version"
        )
    ...
    def test_up_sql_error(self, mock_run):
        ...
        mock_run.return_value = MagicMock(
            returncode=1,
            stdout="",
            stderr="migration failed: syntax error"
        )
    ...
    def test_down_no_migrations(self, mock_run):
        ...
        mock_run.return_value = MagicMock(
            returncode=1,
            stdout="",
            stderr="no migration to rollback"
        )
    ...
    def test_goto_zero(self, mock_run):
        ...
        result = self.wrapper.goto(version=0)
    ...
    def test_full_migration_lifecycle(self, mock_run):
        ...
        # Create migrations
        self.create_test_migration(1, "create_users", 
                                 "CREATE TABLE users (id INTEGER PRIMARY KEY);",
                                 "DROP TABLE users;")
        self.create_test_migration(2, "add_email",
